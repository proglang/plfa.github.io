...
```agda
module plfa.part1.Isomorphism-2026 where
```
...
open Eq using (_≡_; refl; sym ; cong; trans; cong-app)
open Eq.≡-Reasoning
...
≃-trans′ A≃B B≃C =
  ...
  ; from = fromAB ∘ fromBC
  ; from∘to = λ x → (begin
                      (fromAB ∘ fromBC) ((toBC ∘ toAB) x)
                    ≡⟨ cong fromAB (from∘toBC (toAB x)) ⟩
                      fromAB (toAB x)
                    ≡⟨ from∘toAB x ⟩
                      x ∎)
  ; to∘from = to∘from-map
  ...
  where
    open _≃_ A≃B renaming (to to toAB; from to fromAB; from∘to to from∘toAB; to∘from to to∘fromAB)
    open _≃_ B≃C renaming (to to toBC; from to fromBC; from∘to to from∘toBC; to∘from to to∘fromBC)

    to∘from-map : _
    to∘from-map y = trans (cong toBC (to∘fromAB (fromBC y))) (to∘fromBC y)
    -- to∘from-map y
    --   rewrite to∘fromAB (fromBC y) = to∘fromBC y
```
...
≲-antisym′ : ∀ {A B : Set}
  ...
  → (from A≲B ≡ to B≲A)
    -------------------
  → A ≃ B
≲-antisym′ A≲B B≲A eq₁ eq₂ =
  record
  { to = to A≲B
  ; from = to B≲A
  ; from∘to = λ x → trans (cong-app (sym eq₂) (to A≲B x) ) (A≲B .from∘to x)
  ; to∘from = λ y → trans (cong-app eq₁ (to B≲A y)) (B≲A .from∘to y)
  }
