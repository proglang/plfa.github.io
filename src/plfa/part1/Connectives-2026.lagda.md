...
```agda
module plfa.part1.Connectives-2026 where
```
...
* proof for A ∧ B : pair (product) of a proof for A and one for B
* proof for A ∨ B : sum of either a proof A or a proof for B
* proof for ⊤ (truth) : pick a one-element type... (unit type)
* proof for ⊥ (falsity, absurdity) : pick the empty type
* proof for A ⇒ B : function mapping proof for A to proof for B
* proof for ¬ A : encode as A → ⊥

this is constructive (intuitionistic) logic.
