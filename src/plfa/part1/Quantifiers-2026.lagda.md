...
```agda
module plfa.part1.Quantifiers-2026 where
```
...
counter-f : (x : Bool) → x ≡ true ⊎ x ≡ false
counter-f false = inj₂ refl
counter-f true = inj₁ refl
...
counter-example : ¬ (∀ {A : Set} {B C : A → Set}
   → (∀ (x : A) → B x ⊎ C x)
   → (∀ (x : A) → B x) ⊎ (∀ (x : A) → C x))
counter-example st
  with st counter-f
... | inj₁ f = case (f false) of λ()
... | inj₂ g = true≢false (g true)

```
... | inj₁ f = true≢false (sym (f false))
...
_ : Σ ℕ odd
_ = ⟨ 3 , odd-suc (even-suc (odd-suc even-zero)) ⟩

2n+1is-odd : ∀ n → odd (1 + n + n)
2n-is-even : ∀ n → even (n + n)
2n+1is-odd zero = odd-suc even-zero
2n+1is-odd (suc n) = odd-suc (2n-is-even (suc n))
2n-is-even zero = even-zero
2n-is-even (suc n) rewrite +-suc n n = even-suc (2n+1is-odd n)
```
...
counter-arg : Σ Bool (true ≡_) × Σ Bool (false ≡_)
counter-arg = ⟨ ⟨ true , refl ⟩ , ⟨ false , refl ⟩ ⟩

∃-counter : ¬ ( ∀ {A : Set} {B C : A → Set} → (∃[ x ] B x) × (∃[ x ] C x) → ∃[ x ] (B x × C x))
∃-counter f
  with f counter-arg
... | ⟨ x , ⟨ refl , () ⟩ ⟩
```
...
∃¬-implies-¬∀ ⟨ x , ¬Bx ⟩ ∀xB = ¬Bx (∀xB x)

∃¬-counter : ¬ ((x : Bool) → x ≡ true)
∃¬-counter f = true≢false (sym (f false))
