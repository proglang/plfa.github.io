...
```agda
module plfa.part2.DeBruijn-2026 where
```
...
```
    ...
    ∅ ∋ A -- this is absurd, since it does not match any constructor
...
data _—→_ {Γ} : ∀ {A} → (Γ ⊢ A) → (Γ ⊢ A) → Set where
  ...
  ξ-·₁ : ∀ {A B} {L L′ : Γ ⊢ A ⇒ B} {M : Γ ⊢ A}
    → L —→ L′
      ---------------
    → L · M —→ L′ · M

  ξ-·₂ : ∀ {A B} {V : Γ ⊢ A ⇒ B} {M M′ : Γ ⊢ A}
    → Value V
    → M —→ M′
      ---------------
    → V · M —→ V · M′

  β-ƛ : ∀ {A B} {N : Γ , A ⊢ B} {W : Γ ⊢ A}
    → Value W
      --------------------
    → (ƛ N) · W —→ N [ W ]
  ...
  ξ-case : ∀ {A} {L L′ : Γ ⊢ `ℕ} {M : Γ ⊢ A} {N : Γ , `ℕ ⊢ A}
    → L —→ L′
      -------------------------
    → case L M N —→ case L′ M N

  β-zero :  ∀ {A} {M : Γ ⊢ A} {N : Γ , `ℕ ⊢ A}
      -------------------
    → case `zero M N —→ M

  β-suc : ∀ {A} {V : Γ ⊢ `ℕ} {M : Γ ⊢ A} {N : Γ , `ℕ ⊢ A}
    → Value V
      ----------------------------
    → case (`suc V) M N —→ N [ V ]
...
```agda
V¬⟶ : ∀ {Γ A} → {V : Γ ⊢ A} → Value V → ¬ (∃[ M ] (V —→ M))
V¬⟶ (V-suc v) ⟨ M , ξ-suc red ⟩ = V¬⟶ v ⟨ _ , red ⟩
V¬⟶ V-zero ⟨ M , () ⟩

V¬⟶' : ∀ {Γ A} {V M : Γ ⊢ A} → Value V → ¬ (V —→ M)
V¬⟶' (V-suc v) (ξ-suc red) = V¬⟶' v red
...
progress (L · M) | done V-ƛ | done v-M = step (β-ƛ v-M)
-- V-ƛ with progress M
