...
```agda
module plfa.part2.Lambda-2026 where
```
...
Church encoding of the number _n_:
* takes a function f
* argument x
...
plusᶜ ⌈m⌉ ⌈n⌉ = λ s λ z → ⌈m⌉ s (⌈n⌉ s z) ≈ λ s λ z → ⌈m+n⌉ s z

⌈m⌉ sucᶜ `zero ≈ suc^m zero
...
mult′ : ℕ → ℕ → ℕ
mult′ = λ m n → case m of λ{ zero → zero
                           ; (suc m) → n + mult′ m n }
